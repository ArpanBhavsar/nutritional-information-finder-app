...
class _MedicineScannerScreenState extends State<MedicineScannerScreen> {
  ...
  void initState() {
    super.initState();
    _medicineScannerBloc = MedicineScannerBloc(
      ollama: OllamaClient(baseUrl: 'http://10.0.2.2:11434/api'),
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    if (pickedFile != null) {
      _medicineScannerBloc.add(ScanMedicineEvent(pickedFile, useApi: _useApi));
    } else {
      ...
      child: Scaffold(
          ...
          actions: [
            BlocBuilder<MedicineScannerBloc, MedicineScannerState>(
              builder: (context, state) {
                if (state is! MedicineScannerInitial &&
                    state is! MedicineScannerLoading) {
                  return IconButton(
        ...
        body: BlocConsumer<MedicineScannerBloc, MedicineScannerState>(
          ...
          builder: (context, state) {
            ...
            if (state is MedicineScannerFailure && state.image != null) {
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: <Widget>[
                  if (state is MedicineScannerInitial ||
                      state is MedicineScannerImagePicked)
                    SwitchListTile(
                      title: const Text('Use high-accuracy API'),
                  ...
                  if (state is MedicineScannerLoading) ...[
                    ...
                    Text(
                      'Analyzing medicine with ${_useApi ? 'API' : 'local engine'}...',
                    ...
                    Text(
                      'Failed to analyze. ${state.error}',
                      textAlign: TextAlign.center,
                      style: const TextStyle(color: Colors.red, fontSize: 16),
                    ),
                  ] else if (state is MedicineScannerInitial ||
                      state is MedicineScannerImagePicked) ...[
                    const Text(
        ...
        floatingActionButton:
            BlocBuilder<MedicineScannerBloc, MedicineScannerState>(
              builder: (context, state) {
                if (state is MedicineScannerSuccess ||
                    state is MedicineScannerFailure ||
                    state is MedicineScannerInitial ||
                    state is MedicineScannerImagePicked) {
                  if (_isScrolled) {
                    ...
                    String labelText =
                        (state is MedicineScannerSuccess ||
                                state is MedicineScannerFailure ||
                                state is MedicineScannerImagePicked)
